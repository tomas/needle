...
var Needle = {
	...
	request: function(uri, method, data, options, callback){
		...
		var config = {
			...
			follow: options.follow === true ? 20 : options.follow || 0, // true -> 20 redirects
	...
	get_request_opts: function(method, uri, headers, proxy){
		...
		headers["Host"] = proxy ? url.parse(uri).hostname : remote.hostname;
	...
	send_request: function(count, method, uri, config, post_data, callback){
		...
		var request = protocol.request(request_opts, function(response){
			...
			if((response.statusCode == 301 || response.statusCode == 302)
				&& response.headers.location && count <= config.follow)

...
var Needle = {
  ...
  setup_option: function(which, target, option) {

    var opts  = {},
        value = option || defaults[which]; // can be boolean or object (if passed via options)

    if (value) {
      if (typeof value == 'number') { // ok, then map passed value to first key (follow max, open timeout)
        var first_key = Object.keys(nested_defaults[which])[0];
        opts[first_key] = value; // { max: value } or { open: value }
      } else {
        opts = value;
      }

      Object.keys(nested_defaults[which]).forEach(function(key) {
        var config_key = which + '_' + key;
        target[config_key] = typeof opts[key] != 'undefined' ? opts[key] : nested_defaults[which][key];
      })
    }
  ...
  request: function(method, uri, data, options, callback) {
    ...
    config = this.setup_option('follow', config, options.follow);
    config = this.setup_option('timeout', config, options.timeout);
  ...
  send_request: function(count, method, uri, config, post_data, out, callback) {
    ...
    var request = protocol.request(request_opts, function(resp) {
      ...
      // clear read timeout, now set read timeout
    ...
    if (config.open_timeout > 0) {
      timer = setTimeout(function() { request.abort }, config.open_timeout);
    }
...
exports.defaults = function(obj) {
  ...
  for (var key in obj) {
    ...
    // handle follow: { max: 10 }
    if (obj[key].constructor == Object && nested_defaults[key]) {
      nested_defaults[key] = obj[key]; 

    } else if (defaults.hasOwnProperty(key) && typeof obj[key] != 'undefined') {

      // ensure type matches the original
      if (obj[key].constructor.toString() != defaults[key].constructor.toString())

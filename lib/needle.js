...
var Needle = {
  ...
  send_request: function(count, method, uri, config, post_data, callback){
    ...
    var request = protocol.request(request_opts, function(response){
      ...
      var chunks = [], length = 0,
          compressed = /gzip|deflate/.test(response.headers['content-encoding']),
          header = response.headers['content-type'] && response.headers['content-type'].split(';'),
          charset = header && header[1].replace('charset=', '').trim();

      var response_opts = {
        parse_response: config.parse_response,
        content_type: header && header[0],
        charset: charset,
        utf8: charset.match(/utf-?8$/i) && !compressed
      }
      ...
      response.on('end', function(){
        ...
        if (typeof unzip != 'undefined' && compressed)
          unzip(body, function(err, buff){
            self.response_end(response_opts, response, buff, callback);
          });
        else
  ...
  response_end: function(opts, response, body, callback){
    ...
    if (opts.parse_response && parsers[opts.content_type]) {
      parsers[opts.content_type](body, function(err, result){

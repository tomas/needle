  ...
  request: function(method, uri, data, options, callback) {
    ...
    if (data) {
      if (method.toUpperCase() == 'GET') { // build query string and append to URI
        ...
        multipart.stream(data, function(err, stream, length) {
          if (err) throw(err);

          config.headers['Content-Type']   = 'multipart/form-data; boundary=' + stream.getBoundary();
          config.headers['Content-Length'] = length;
          self.send_request(1, method, uri, config, stream, out, callback);
        });

...
var Needle = {
  ...
  request: function(method, uri, data, options, callback) {
    ...
    for (var h in defaults.headers)
      config.headers[h] = defaults.headers[h];
...
exports.defaults = function(obj) {
  ...
  if (obj.headers && Object.prototype.toString.call(obj.headers).slice(8, -1) == 'Object') {
    defaults.headers = {};
    for (var key in obj.headers) {
      if (typeof obj.headers[key] == 'string' || typeof obj.headers[key] == 'number')
        defaults.headers[key] = obj.headers[key];
    }
  }

...
var Needle = {
  ...
  send_request: function(count, method, uri, config, post_data, callback){
    ...
    debug('Making request', request_opts);
    ...
    // response.setEncoding(response_opts.utf8 ? 'utf8' : 'binary');
      ...
      resp.on('end', function(){
        ...
        if (!compressed || typeof unzip == 'undefined')
          return self.response_end(response_opts, resp, body, callback);

        unzip(body, function(err, buff){
          var out = err ? body : buff; // if we hit a bump when unzipping, return original body
          self.response_end(response_opts, resp, out, callback);
        });
  ...
  response_end: function(opts, response, body, callback){
    ...
    var handle_output = function(err, data){
      ...
      fs.writeFile(opts.output, data, function(e){
...
exports.request = function(type, uri, data, options, callback) {
  return Needle.request(uri, type.toUpperCase(), data, options, callback);
}

...
var Needle = {
	...
	request: function(uri, method, data, options, callback){
		...
		var config = {
			...
			follow: options.follow === true ? 10 : options.follow || 0, // true -> 10 redirects
		...
		if(request_compressed) config.headers['Accept-Encoding'] = 'gzip,deflate';
	...
	get_request_opts: function(method, uri, headers, proxy){

		var remote = proxy ? url.parse(proxy) : url.parse(uri);
		var port = remote.port || (remote.protocol == 'https:' ? 443 : 80);
		headers["Host"] = (proxy ? url.parse(uri).hostname : remote.hostname) + ":" + port;

		var opts = {
			host: remote.hostname,
			port: port,
			protocol: remote.protocol,
			path: proxy ? uri : remote.pathname + (remote.search || ''),
			method: method,
			headers: headers
		};
	...
	send_request: function(count, method, uri, config, post_data, callback){
		...
		var request_opts = this.get_request_opts(method, uri, config.headers, config.proxy);
		if (typeof config.agent != 'undefined') request_opts.agent = config.agent;

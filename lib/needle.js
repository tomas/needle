...
Needle.prototype.setup = function(uri, options) {
  ...
  var config = {
    ...
    proxy            : options.proxy,
    proxy_after_auth : options.proxy_after_auth || false,
  ...
  if(config.proxy_after_auth && (config.headers['Proxy-Authorization'] || config.credentials)) {
    config._proxy = config.proxy;
    delete config.proxy;
  }
    ...
    if (resp.statusCode == 401 && headers['www-authenticate'] && config.credentials) {
      if (!config.headers['Authorization']) { // only if authentication hasn't been sent
        ...
        if (auth_header) {
          ...
          if(config._proxy && config.proxy_after_auth) config.proxy = config._proxy;

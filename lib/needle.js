...
var Needle = {
  ...
  html_charset_finders: [
    function(body){
      return body.match(/<meta[^>]+content-type[^>]+charset=([a-z0-9-]+)/im)[1] || null;
    },
    function(body){
      return body.match(/<meta[^>]+charset=['"]?([a-z0-9-]+)/im)[1] || null;
    },
    function(body){
      return body.match(/<?xml[^>]+encoding=['"]?([a-z0-9-]+)/im)[1] || null;
    }
  ],

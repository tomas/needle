    ...
    var request = protocol.request(request_opts, function(resp){
      ...
      if ([301, 302].indexOf(resp.statusCode) != -1 && resp.headers.location){
        if (count <= config.follow)
          return self.send_request(++count, 'GET', resp.headers.location, config, null, callback);
      ...
      var response_opts = {
        ...
        parse: config.parse_response, // parse JSON
        decode: (mime.type.indexOf('text') != -1) && config.decode_response,
        content_type: mime.type,
        charset: mime.charset,
        utf8: mime.charset && !!mime.charset.match(/utf-?8$/i)
      }

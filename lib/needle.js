...
var defaults = {
  ...
  proxy                   : null,
  host_ip                 : null, // can be either a specific ip, an array or 'all'
...
Needle.prototype.setup = function(uri, options) {
  ...
  var config = {
    ...
    host_ip   : get_option('host_ip', defaults.host_ip),
...
Needle.prototype.start = function() {
  ...
  function next(body) {
    ...
    self.resolve_and_send(1, method, uri, config, body, out, callback);
  }
...
Needle.prototype.get_request_opts = function(method, uri, config) {
  ...
  opts.host     = config.host_ip || remote.hostname;
...
Needle.prototype.get_addresses = function(uri, config, cb) {
  if (config.host_ip == 'all')
    return dns.lookup(url.parse(uri).hostname, { all: true }, cb);

  // ok, so we got an array. just build a list of objects with the given IPs.
  var list = config.host_ip.map(function(ip) {
    return { address: ip };
  })
...
Needle.prototype.resolve_and_send = function(method, uri, config, post_data, out, callback) {

  // if no host IP or a specific one was given, go for it.
  if (!config.host_ip || ~config.host_ip.indexOf('.'))
    return this.send_request(1, method, uri, config, post_data, out, callback);

  // ok, so the master wants us to connect sequentially to a list of IPs. let's do it then.
  var address, self = this;
  this.get_addresses(uri, config, function(err, addresses) {
    if (err || !addresses.length)
        throw err || new Error('No addresses found for ' + uri);

    function next() {
      config.host_ip     = addresses.shift();
      config.has_next_ip = addresses.length > 0;
      self.send_request(1, method, uri, config, post_data, out, callback);
    }

    out.on('_next_ip', next)
    next();
  })
...
Needle.prototype.send_request = function(count, method, uri, config, post_data, out, callback) {
  ...
  function done(err, resp, body) {
    ...
    if (err && config.has_next_ip) {
      debug('Got ' + err.code + ' error, trying next IP address.');
      return out.emit('_next_ip');
    }

    if (callback)
      return callback(err, resp, body);

    // if no callback, this can only be called from had_error() below.
    if (!err) throw new Error('Something is not right. Please submit a bug report!');
    out.emit('end', err);
  }

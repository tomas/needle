...
var fs          = require('fs'),
    ...
    pump        = require('pump'), // stream.pipeline for older versions of node
...
function pump_streams(streams, cb) {
  if (stream.pipeline) // v10 or higher
    return stream.pipeline.apply(null, streams.concat(cb));

  pump.apply(null, streams.concat(cb));
}
...
Needle.prototype.send_request = function(count, method, uri, config, post_data, out, callback) {
  ...
  var request = protocol.request(request_opts, function(resp) {
    ...
    pump_streams([resp].concat(pipeline), function(err) {
      if (err) debug(err)
      // if (err && err.code == 'ERR_STREAM_PREMATURE_CLOSE') request.destroy();
    });

...
var Needle = {
  ...
  request: function(method, uri, data, options, callback) {
    ...
    var config = {
      ...
      follow_method   : options.follow_method || defaults.follow_method,
    ...
    var request = protocol.request(request_opts, function(resp) {
      ...
      if ([301, 302, 303].indexOf(resp.statusCode) != -1 && headers.location) {
        if (count <= config.follow) {
          ...
          return self.send_request(++count, config.follow_method ? method : 'GET', url.resolve(uri, headers.location), config, config.follow_method ? post_data : null, out, callback);

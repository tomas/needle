...
Needle.prototype.setup = function(uri, options) {
  ...
  for (var h in options.headers) {
    ...
    // clear options.headers of any `undefined` values
    // see https://github.com/tomas/needle/issues/258
    // e.g. this allows callers to blank out the User-Agent header,
    // by passing in User-Agent -> undefined
    if (options.headers[h] === undefined) {
      delete config.headers[h.toLowerCase()]
    }
  }

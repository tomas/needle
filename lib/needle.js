...
var Needle = {
  ...
  request: function(uri, method, data, options, callback){
    ...
    var config = {
      ...
      ssl: options.ssl === true ? true : false,
      charset: options.charset,
      real_path: options.real_path === true ? true : false
    }
  ...
  get_request_opts: function(method, uri, config){
    ...
    opts.protocol = config.ssl ? 'https:' : remote.protocol;
    opts.host     = remote.hostname;
    opts.port     = remote.port || (remote.protocol == 'https:' || config.ssl ? 443 : 80);
  ...
  send_request: function(count, method, uri, config, post_data, callback){
    ...
    var request = protocol.request(request_opts, function(resp){
      ...
      if(config.output && config.real_path){
        var real_path = resp.client._httpMessage.path;
        var dir = config.output.match(/(.*\/).*/) ? config.output.match(/(.*\/).*/)[1] : config.output;
        if(real_path[real_path.length-1]=='/')real_path = real_path.slice(0,-1);
        var filename = real_path.match(/.*\/(.*)/) ? real_path.match(/.*\/(.*)/)[1] : real_path;
        var mime_ext = _mime.extension(mime.type);
        if(mime.type && mime_ext && filename.indexOf('.' + mime_ext) < 0 && _mime.lookup(filename) != mime.type )filename += '.' + mime_ext;
        else if(mime.type && !_mime.lookup(filename)){filename = filename + '.' + mime.type.match(/\/(.*)/)[1];}
        if(filename.indexOf('?')>0)filename = filename.slice(0, filename.indexOf('?'));
        if(!filename.length){
          if (debug) console.log('Cannot resolve real path for: ' + uri);
          real_path = config.output;
        }else{
